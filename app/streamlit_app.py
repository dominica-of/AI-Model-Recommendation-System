...
if page == "Model Recommender":
    ...
    input_data_filter = st.sidebar.multiselect("Input Data", input_data_options, default=input_data_options)

    st.sidebar.markdown("🛈 **Filter by ML Task**")
    task_filter = st.sidebar.multiselect("Task", task_options, default=task_options)
    ...
    st.title("Scoring Engine Explanation")
    st.image("scoring_flowchart.png", caption="Model Evaluation and Scoring Flowchart")

    with st.expander("See Scoring Formulas"):
        st.latex(r"Performance\ Score = 0.4 \times (1 - Normalized\ Inference\ Time) + 0.3 \times (1 - Normalized\ Model\ Size) + 0.3 \times (1 - Normalized\ Memory\ Requirement)")
        ...
        st.latex(r"Overall\ Score = 0.3 \times Performance + 0.2 \times Popularity + 0.15 \times Licensing + 0.15 \times Hardware + 0.1 \times Documentation + 0.1 \times Institution")

    st.image("weight_contribution.png", caption="Bar chart of score factor weights")

    st.subheader("📋 Impact Table")
    impact_data = [
        ["Performance", "Low inference time, small model size", "High inference time, large model", "Prioritizes faster, cheaper models"],
